...
namespace muscshop.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult SignUp(User newUser)
        {
            ...
            var text = $"To finish registraition go to: {urlHost}/Account/Confirmation/{user.Confirmation}";
        ...
        private ActionResult SendRecovery2(User user)
        {
            Uri uri = new Uri(Request.Url.AbsoluteUri);
            var urlHost = uri.Scheme + Uri.SchemeDelimiter + uri.Host + ":" + uri.Port;
            var text = $"To restore your password go to: {urlHost}/Account/Recovery/{user.PassRecovery}";
        ...
        {
            System.Net.Mail.MailMessage mail = new System.Net.Mail.MailMessage();
            mail.From = new System.Net.Mail.MailAddress("gransareg@gmail.com");
            mail.To.Add(email);
            mail.Subject = "Registration Confirm";
            mail.Body = text;
            mail.IsBodyHtml = true;
